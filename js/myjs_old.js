...
// set default pID
if (!petitionId){petitionId=241584};

//setlink vars
document.querySelector('.plink').setAttribute('href','https://petition.parliament.uk/petitions/'+petitionId);
document.querySelector('.json').setAttribute('href','https://petition.parliament.uk/petitions/'+petitionId+'.json');
document.querySelector('.sign').setAttribute('href','https://petition.parliament.uk/petitions/'+petitionId+'/signatures/new');
...
var loadPage = function (myJson, x=false){
    ...
    var i;
	 ...
	 sigHistory.push(totalSigs);
		...
		if (sigHistory.length >0)  { 3.
				...
				if (sigJump < 1) {
					document.querySelector('.jump').innerHTML = "Jump: <strong id ='jumpTxt' class='red'>" +sigJump +"</strong> in the past 10 seconds";
				//document.getElementById("jumpTxt").style.fontSize = sigJump + 10 + "px";
				sigHistory.shift();
				} else {
						document.querySelector('.jump').innerHTML = "Jump: <strong id ='jumpTxt' class='green'>+" +sigJump +"</strong> in the past 10 seconds";
					//document.getElementById("jumpTxt").style.fontSize = sigJump + 10 + "px";
					sigHistory.shift();
					}
				}
    ...
    console.log("Home Nations Total: "+homeTotal);
		...
		//render uk only pc
		document.querySelector('.ukOnlyPc').innerHTML = noC((ukOnly/(totalSig/100)).toFixed(2) + "%");

		// render countries sig pc
		document.querySelector('.scoPc').innerHTML = ((sco/(totalSig/100)).toFixed(2) + "%");
		document.querySelector('.engPc').innerHTML = ((eng/(totalSig/100)).toFixed(2) + "%");
		document.querySelector('.irePc').innerHTML = ((ire/(totalSig/100)).toFixed(2) + "%");
		document.querySelector('.walPc').innerHTML = ((wal/(totalSig/100)).toFixed(2) + "%");
		
		// render non uk
		document.querySelector('.nonUkPc').innerHTML = noC((nonUK/(totalSig/100)).toFixed(2) + "%");
...
var table = new Tabulator("#example-table", {
		...
		columns:[
				...
				{title:"Count", field:"signature_count", formatter:"money", formatterParams:{precision:false}, widthGrow:2, sorter:"number"},				
				{title:"%", width:75, field:"signature_count", download:false, widthGrow:1, formatter:function(cell, formatterParams, onRendered){
...
$("#download-xlsx").click(function(){
    table.download("xlsx", "petition_data_by_country - "+Date()+".xlsx", {sheetName:"Petition Data"});
});
